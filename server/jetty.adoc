...
[arabic]
. 参考http://wiki.eclipse.org/Jetty/Howto/High_Load[Jetty/Howto/High++_++Load],修改系统参数
. 从http://www.eclipse.org/jetty/[官网] 下载，并上传的指定的服务器上。
. 解压
+
[source,bash]
----
...
. 修改 jetty 的全局性配置
[arabic]
.. `vi $JETTY++_++HOME/etc/jetty.xml`, 修改92行左右，启用 *ForwardedRequestCustomizer*，该配置允许使用 HaProxy, Nginx等反向代理设置 `X-Real-IP`, `X-Forwarded-For`, `X-Forwarded-Proto` 等 http 请求头
.. `vi $JETTY++_++HOME/etc/jetty-logging.xml`, 修改19行左右，修改时区 `GMT` -++>++ `GMT{plus}8`. 这会使得 jetty 生成的日志备份文件 `yyyy++_++mm++_++dd.stderrout.log.HHmmssSSS"` 的后缀能与我们的时区相一致。
.. `vi $JETTY++_++HOME/bin/jetty.sh` ，为方便非root用户可以执行该脚本，修改以下配置。
+
[source,bash]
----
...
----
.. `vi $JETTY++_++HOME/bin/jetty.sh` ，防止Jenkins 通过SSH执行时，因stdout而超时。
+
[source,bash]
----
...
# 修改 463 行左右，修改 `else\n "${RUN_CMD[@]}" &` 为：
  ...
  "${RUN_CMD[@]}" > /dev/null &
----
. 创建某个工程（这里是 my-app）特定的 `jetty.base`
[arabic]
.. 创建目录
+
[source,bash]
...
----
. 创建某个工程（这里是 my-app）特定的配置文件 ： `vi /etc/default/my-app` 。
+
[source,bash]
----
#!/bin/bash
...
[arabic]
.. 该文件是shell脚本。
.. 该路径只有root用户才可读取。非root用户需要到 `$HOME/etc/default/` 目录下读取。
.. JVM参数中关于远程debug的配置，请修改端口号，防止冲突。如有必要，请移除远程debug的配置。
. 创建某个工程（这里是 lizi-cas）特定的 init.d 脚本
+
[source,bash]
----
...
[arabic]
. nginx https 反向代理。浏览器与nginx之间是https，但nginx与tomcat/jetty之间是普通的http。nginx配置文件如下所示：
+
[source,nginx]
----
...
----
. 使用 jetty-maven-plugin 对外提供 https服务。因为 lizi-cas 使用 maven 的 war overlay方式开发，所以debug时，使用 jetty-maven-plugin 插件会很方便。
[arabic]
.. 需要从jetty压缩包中copy所需的 etc/jetty++*++xml 到 lizi-cas 工程中，并略做修改，比如：修改 jetty.xml, 启用 ForwardedRequestCustomizer。
.. 修改 pom.xml
+
[source,xml]
----
...
----
. 运行 jetty-maven-plugin 插件时，连接其他 https 服务，并验证证书。修改 pom.xml，追加参数
+
[source,xml]
----
<plugin>
...
----
. 通过 `grails run-app` 运行时，连接其他 https 服务，并验证证书。
+
[source,bash]
----
# 方式一 ： 在命令行修改参数
...
----
. 独立运行 jetty 时，连接其他 https 服务，并验证证书。`vi /etc/default/xxxAppName` :
+
[source,bash]
----
JAVA_OPTIONS="-server \
