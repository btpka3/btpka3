...
因此，服务器上决定使用 http://tengine.taobao.org/[Tengine] 来取代 Nginx 官方版。
...
[arabic]
. 新建 systemd 所需的 service 文件： `vi /usr/lib/systemd/system/tengine.service` : 注意： Ubuntu 16.04 LTS 版本文件路径为 `vi /lib/systemd/system/tengine.service`
+
....
[Unit]
...
[arabic]
. 下载 init.d 脚本。从 http://wiki.nginx.org/InitScripts[这里] 为 CentOS 下载 `Red Hat /etc/init.d/nginx`, 并保存到 `/etc/init.d/nginx`
. 修改 `/etc/init.d/nginx` ,
+
[source,bash]
----
...
参考http://wiki.nginx.org/Upstart[这里]
