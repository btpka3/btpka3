基于 nginx 的产品: https://openresty.org/en/[OpenResty]
...
pam : see http://www.doublecloud.org/2014/01/nginx-with-pam-authentication/[here]

== 参考

* 《https://yq.aliyun.com/articles/26635?&utm_campaign=sys&utm_medium=market&utm_source=edm_email&msctype=email&mscmsgid=117816050400209379&[NGINX引入线程池 性能提升9倍]》

== 状态统计

* http://nginx.org/en/docs/http/ngx_http_stub_status_module.html[ngx++_++http++_++stub++_++status++_++module]
+
只提供了基础统计信息，比如：
+
[source,text]
...
----
* http://nginx.org/en/docs/http/ngx_http_status_module.html[ngx++_++http++_++status++_++module]
+
貌似比较强大，但是收费。
* https://github.com/vozlt/nginx-module-vts[nginx-module-vts]
* https://github.com/zls0424/ngx_req_status[ngx++_++req++_++status]
+
只是纯文本显示。可以统计URL。
* https://github.com/yandex-sysmon/nginx-lua-stats[nginx-lua-stats]
...
http://wiki.nginx.org/Modules[Nginx Modules]、 http://wiki.nginx.org/3rdPartyModules[Nginx 3rdPartyModules]
...
https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Strict-Transport-Security[HTTP Strict Transport Security]

[arabic]
. 只有 通过 https 协议设置才有效，http 无效。
...
http://nginx.org/en/docs/http/ngx_http_upstream_module.html[ngx++_++http++_++upstream++_++module] 3rd https://code.google.com/p/nginx-upstream-jvm-route/[nginx-upstream-jvm-route] 3rd https://code.google.com/p/nginx-sticky-module[nginx-sticky-module]

关于反向代理 Jetty 的 http://eclipse.org/jetty/documentation/current/configuring-connectors.html#jetty-connectors-http-configuration[X-Forwarded-For] 的配置。
...
http://heartbleed.com/[heartbleed]、 http://nginx.com/blog/nginx-and-the-heartbleed-vulnerability[nginx and heartbleed]
...
http://nginx.org/en/docs/http/ngx_http_fastcgi_module.html[proxy++_++next++_++upstream] http://nginx.org/en/docs/http/ngx_http_fastcgi_module.html#fastcgi_next_upstream[fastcgi++_++next++_++upstream] https://github.com/cep21/healthcheck_nginx_upstreams[healthcheck++_++nginx++_++upstreams] 3rd https://github.com/yaoweibin/nginx_upstream_check_module[nginx++_++upstream++_++check++_++module]

=== https 反向代理

http://www.cyberciti.biz/faq/howto-linux-unix-setup-nginx-ssl-proxy/[1] http://webapp.org.ua/sysadmin/setting-up-nginx-ssl-reverse-proxy-for-tomcat/[2]
...
http://wiki.nginx.org/Install[参考]

== 安装

[arabic]
. 新增nginx的yum仓库，`vi /etc/yum.repos.d/nginx.repo`
+
[source,bash]
----
[nginx]
...
----
. 为sso配置。该示例与link:centos-tomcat[tomcat 虚拟主机]的配置相呼应
+
[source,bash]
----
...
使用Nginx Plus {plus} 提供的 http://nginx.org/en/docs/http/ngx_http_upstream_module.html#health_check[health++_++check] 指令

使用 http://wiki.nginx.org/HttpHealthcheckModule[ngx++_++http++_++healthcheck++_++module]
...
效果：访问 `/api/test/index` 实际是代理访问了 `/test/index`。 但是，注意：此时返回的HTML的URL路径普遍都有问题。

=== 负载均衡算法

[width="100%",cols="30%,18%,22%,22%,8%",options="header",]
|===
|derective |nginx version |Only in nginx plus? |tengine version |memo
|hash |1.7.2{plus} | | |
|ip++_++hash | | | |
|least++_++conn |1.3.1{plus}, 1.2.2{plus} | | |最小连接数
|least++_++time |1.7.10{plus} |Yes | |最小平均响应时间
|sticky |1.5.7 |Yes | |
|queue |1.5.12 |Yes | |
|===

参考：ngx++_++http++_++upstream++_++module++]++(http://nginx.org/en/docs/http/ngx++_++http++_++upstream++_++module.html#hash)

=== 子域名跳转

使得外部访问 `http://www.test.me/ask/xxx` 时都跳转到 `http://ask.test.me/xxx` 。 但是内部app都只有一个，故内部访问 http://ask.test.me/xxx 时，还是在访问 http://www.test.me/ask/xxx
...
参考：http://nginx.org/en/docs/stream/ngx_stream_upstream_module.html#upstream[ngx++_++stream++_++upstream++_++module]
