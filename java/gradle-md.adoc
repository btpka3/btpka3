...
* https://reflectoring.io/maven-scopes-gradle-configurations/#api[Maven Scopes and Gradle Configurations Explained]
* https://docs.gradle.org/current/userguide/java_library_plugin.html#sec:java_library_configurations_graph[The Java Library Plugin]
* https://docs.gradle.org/current/userguide/java_plugin.html#java_plugin[java pluign]
* https://blog.gradle.org/alignment-with-gradle-module-metadata[Automatically align Dependencies with Platforms and Gradle Module Metadata]
* https://docs.gradle.org/current/userguide/component_metadata_rules.html[Fixing metadata with component metadata rules]

假设我们要开发 java 库： - my-lib-api : 不经常变动的 API 层（包含 interface 和相关 出参、入参定义 - my-lib-api-http : 通过 HTTP 调用的相关实现和封装 - my-lib-api-mq : 通过 message queue 调用的相关实现和封装 - my-lib-client : 业务方直接调用，封装了对 API 的调用。

假设我们使用 gradle 的 java-library 插件，开发二方包给 别人使用, 声明了以下依赖： 开发 my-lib-client 时配置以下依赖：

* api
** 当前gradle工程中，编译，运行时均可用。但第三方使用的时候，也是编译、运行时均可用 注意：只有当使用了 gradle java-library 插件的时候，才可以使用。
...
** 说明：我们的 my-lib-client 内部使用的内部的打印日志的用的， 如果业务方运行时使用 logback， 则可以通过配置，打印 JOSN 日志，但如果业务方不使用 logback, 而使用了 log4j ， 则可以使用另外的配置文件 打印普通的 日志文件
* compileOnly
...
参考：http://depressiverobot.com/2016/02/05/intellij-path.html[1]
...
http://stackoverflow.com/a/34786210/533317[ref]
...
maven可以通过 settings.xml 进行全局配置，比如本地仓库的位置，mirror等。 那Gradle如何进行类似设置？——可以使用 https://docs.gradle.org/current/userguide/init_scripts.html[init script]
...
* https://docs.gradle.org/current/userguide/custom_plugins.html[Developing Custom Gradle Plugins]
