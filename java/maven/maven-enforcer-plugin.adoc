...
----
mvn enfocer:enforce
mvn org.apache.maven.plugins:maven-enforcer-plugin:3.6.2:enforce -Denforcer.rules=requireReleaseDeps -Denforcer.failFast=true
mvnDebug org.apache.maven.plugins:maven-enforcer-plugin:3.6.2:enforce -Denforcer.rules=requireReleaseDeps -Denforcer.failFast=true
...
bannedPlugins           # 禁用
dependencyConvergence   # 确保依赖的版本号一致，不冲突。如果冲突，可以发现然后并用 dependencyManagement 来处理
