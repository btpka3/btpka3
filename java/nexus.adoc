...
[arabic]
. 创建所需执行的非root用户，比如： `nexus`。
. 下载最新版本，并解压。比如：解压到 `/home/nexus`
. 修改 `/etc/profile.d/xxx.sh` ，追加/修改以下环境变量
+
[source,bash]
----
...
----
. 修改 `$++{++NEXUS++_++HOME}/conf/nexus.properties`，按需修改以下配置项
+
[source,ini]
----
...
----
. 现在可以通过 `service nexus start` 启动了，访问URL为 `http://localhost:20010/`
. 为了使用自定义域名 `mvn.lizi.com` 访问，需要为Nginx增加以下配置项：
+
....
upstream nexus {
...
....
. 之后通过URL `http://localhost:20010/` 登录，默认管理员 `admin`，密码 `admin123` 登录，并修改：
+
....
...
[width="100%",cols="41%,38%,14%,7%",options="header",]
|===
