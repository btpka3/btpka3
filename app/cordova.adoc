...
最新的 cordova (6.3.0) 使用的是 cordova-ios@4.2.0，cordova-android@5.2.1。 安卓开发的话，支持 https://cordova.apache.org/docs/en/latest/guide/platforms/android/[14-23] 的安卓 API(Android 4.0~6.0)。

== 如何真机调试？

参考 http://blog.csdn.net/freshlover/article/details/42528643[这里]。

注意：初次使用时，会白屏，因为谷歌的服务器被墙了。可以使用 XX-NET 翻墙一下就OK了。

== 设置 android webview 可调试

参考 https://developers.google.com/chrome-developer-tools/docs/remote-debugging#configure-webview[1]
