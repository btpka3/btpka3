...
** 使用 https://github.com/btpka3/btpka3/blob/master/java/jstat.md[jstat] 分析内存使用状况 `watch $++{++JAVA++_++HOME}/bin/jstat -gcutil $++{++java进程PID}`
...
* jvm 上一定要手动明确限定内存参数。 目前 docker 容器都没有限定内存的，docker 容器内的 java 应用不指定 内存参数，会默认使用宿主机的最大内存做调整的。
...
+
如果要监控的 jvm 位于云上环境内外，可以临时开启 ssh 隧道， 使之访问本地(localhost)特定端口，就是访问远程内网服务器的特定端口:
+
[source,bash]
----
# 在SSH client端执行
...
----
* 远程 debug。需要仿照追加 jvm 参数 `-Xdebug -Xrunjdwp:transport=dt++_++socket,address=10821,server=y,suspend=n`
...
----
+
dump 出来的文件，推荐使用 http://www.eclipse.org/mat/[Eclipse Memory Analyzer], 内存分析示例
+
.MAT 分析结果：内存泄露
image::mat-1.png[MAT 分析结果：内存泄露]
* 在云主机上，增加https://help.aliyun.com/document_detail/63275.html[阿里云自定义监控]，至少判断 服务是否有挂掉
** 通过 curl 命令 相关数据。如果觉得 shell 编写麻烦的话，就用最小化的 spring boot 应用取代(建议)。 通过 http 请求，判断使用是否启动起来。
** 通过 cron 任务 定时触发
** 通过 钉钉机器人 发送告警
** PS: 如果可以，可以监控 jvm 内存使用状况
* 增加 docker 清理的定时任务，参考 https://docs.docker.com/engine/reference/commandline/system_prune/#description[docker system prune]
