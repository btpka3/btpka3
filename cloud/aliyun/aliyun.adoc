...
* link:https://github.com/aliyun/aliyun-cli[github : aliyun/aliyun-cli]
* link:https://www.alibabacloud.com/help/zh/cli/what-is-alibaba-cloud-cli[什么是阿里云 CLI]
...
aliyun configure set \
    ...
    --access-key-secret SK_XXX



# 示例配置文件
ll ~/.aliyun/config.json
{
	"current": "xxx_profile",
	"profiles": [
		{
...
####################################################### SLS  : GetLogs
# aliyun cli 的帮助文档默认携带了 颜色，但其颜色对黑色主题的终端不友好，可以通过 NO_COLOR=true 禁止带颜色输出
NO_COLOR=true aliyun sls GetLogs --help
aliyun -p aigc_yun_cn_bj_sls sls GetLogs \
 --project gong9-exception-aigc \
 --logstore gong9-log-raw-error \
...
aliyun -p aigc_yun_cn_bj_sls sls GetLogsV2 \
 --project gong9-exception-aigc \
 --logstore gong9-log-raw-error \
 --body "${query_param}" \
  | jq

# 《使用aliyun mcp-proxy代理OpenAPI MCP Server》 https://www.alibabacloud.com/help/zh/cli/use-aliyun-mcp-proxy-agent-openapi-mcp-server
# 《OAuth应用概览》https://help.aliyun.com/zh/ram/overview-of-oauth-applications
# 《容器服务 Kubernetes 版 ACK：CLI集成示例》https://help.aliyun.com/zh/ack/ack-managed-and-ack-dedicated/developer-reference/cli-integration-example
aliyun mcp-proxy --help
----
