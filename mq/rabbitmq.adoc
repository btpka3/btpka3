...
RabbitMQ是使用Erlang开发，因此需要先安装Erlang。具体要使用哪个版本。则需要看一下http://www.rabbitmq.com/which-erlang.html[这里]了。最低需要 R13B03。

PRM 安装请参考https://www.erlang-solutions.com/downloads/download-erlang-otp[这里]。提示：请确 CentOS 上已经安装了 EPEL.

虽然测试环境的都是 CentOs 6.x系列，而生产环境则还有 CentOs 5.x 系列。 通过https://fedoraproject.org/wiki/Erlang[这里]可以得知，CentOS 5.x下的EPEL只支持到 R12B-5，版本太低了。 如果要使用RabbitMq集群，则使用的RabbitMQ、Erlang需要一致。因此，为了方便起见，Erlang还是使用源码编译安装。

从http://www.erlang.org/download.html[这里]下载 otp++_++src++_++17.1.tar.gz。解压后需要先阅读一下 `README.md` 和 `HOWTO/INSTALL.md`
...
参考http://www.rabbitmq.com/download.html[这里]，安装 *Installing on Generic Unix*, 基本上解压后就可以直接运行了。
...
使用yum/rpm 安装，请参考http://www.rabbitmq.com/install-rpm.html[这里]。
...
参考官方文档http://www.rabbitmq.com/clustering.html[clustering] 和http://www.rabbitmq.com/ha.html[HA]。非官方总结可以参考http://88250.b3log.org/rabbitmq-clustering-ha[这里]。 下面以服务器 s83 和 s85 创建集群为例，进行步骤介绍
...
开启相关插件. http://www.rabbitmq.com/management.html[Management]、 http://www.rabbitmq.com/federation.html[federation]
