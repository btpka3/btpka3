...
在 vm 内,禁止 '`Alt{plus}tab`' 在vm内切换窗口:
...
* 安装Ubuntu仓库中的版本 `apt-cache search virtualbox   sudo apt-get install virtualbox virtualbox-guest-additions-iso`
* 安装官方渠道的最新版本
+
....
...
.. `echo vboxsf ++>>++ /etc/moudules` 确保 kernel 提前加载所需模块
.. `vi /etc/fstab`, 内容如下:
+
....
...
https://www.virtualbox.org/wiki/Linux_Downloads[查看这里]

=== 网络连接

https://www.virtualbox.org/manual/ch06.html#network++_++hostonly
...
[width="99%",cols="27%,9%,25%,14%,25%",options="header",]
|===
...
* VirtualBox : File : Tools : Network Manager 新建一个 【Host-only Networks】
** Name : HostNetWork
** Mask : 255.255.255.0
** Lower Bound : 192.168.56.1
** Upper Bound : 192.168.56.199 # 注意：On Linux, macOS and Solaris Oracle VM VirtualBox will only allow IP addresses # in 192.168.56.0/21 range to be assigned to host-only adapters. # ++[++192.168.56.++*++ ~ 192.168.63.++*]++ # 如果想用其他ip段，则需要修改文件: /etc/vbox/networks.conf # 创建完成后 ifconfig 的结果会多一个 vmenet0 的虚拟网卡
...
http://helpdeskgeek.com/virtualization/virtualbox-share-folder-host-guest/[参考]

这里以Host系统为Windows，Guest系统为CentOS为例进行讲解：

[arabic]
. 用虚拟光驱加载 VirtualBox 根目录下的 `VBoxGuestAdditions.iso`
. 启动Guest系统，`设备` 菜单 -++>++ 分配光驱 -++>++ 选择第一步加载的虚拟光驱
. 在Guest系统（CentOS）中加载cdrom，
+
[source,bash]
----
...
http://tecadmin.net/how-to-boot-from-usb-drive-in-virtualbox-on-linux/[参考]
