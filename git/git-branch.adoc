...
格式： major.minor.build ++*++ major : 主版本号 ++*++ minor : 子版本号 ++*++ build : 补丁版本号。 major或minor变更后第一次发布时，该值为0。 major和minor均无变化时，若有新bug需要修正时，该值{plus}1。 参考: http://semver.org/lang/zh-CN/[semver]

== GIT分支管理规则

=== Git tag 规则

tag名称： vlatexmath:[{major}.]++{++minor}
...
* 每个分支组都包含2个分支
** vlatexmath:[{major}.]++{++minor}-dev : 开发分支。（不适用于最新版本）
** vlatexmath:[{major}.]++{++minor}-test : 测试分支。（始终由开发分支push到测试分支、测试通过后在该测试分支节点上打tag）
