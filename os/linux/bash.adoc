...
----
# 1. 查看当前进程的 shell
# 输出如：/bin/bash 或 /usr/bin/zsh
# 注意：这是登录 shell，不一定是当前正在运行的 shell
echo $SHELL

# 2. 查看当前实际运行的 shell（推荐）
# 输出如：bash / zsh / fish / dash
ps -p $$ -o comm=
...
# 4. 利用 shell 内置变量
# 输出如：bash / zsh / -bash（带 - 表示登录 shell）
echo $0
...
# 压缩包内单文件替换
zip -u xxx.zip file  # 但若文件时间未变，可能不更新
# zip 没有替换选项，等同于先删除，再添加
zip -d archive.zip "path/to/file"       # 删除
zip archive.zip "config/app.properties" # 添加，不支持指定entry路径，故需要于entry相同的目录结构。

# 将本地 settings.json 添加为 ZIP 内 myapp/config/settings.json
#-C dir path：切换到 dir 目录，将 path（相对于 dir）添加到归档
#要求：/path/to/base/dir/myapp/config/settings.json 必须存在
#适用场景：已有目录结构，或配合 ln -s 创建软链接（需注意 jar 默认不跟随软链接）
jar uf app.zip -C /path/to/base/dir myapp/config/settings.json
----
...
[source,shell]
----
# 方式一： 管道后的命令比较短，无换行时，可以直接在第一个 EOF后使用管道命令 和 后续命令
cat <<EOF | jq .
{
  "name1"   : "$HOME",
  "name2"   : 222
...
# 方式二： 需要多级管道，管道命令较长，较复杂，有换行时，可以 第一个 EOF 仅仅写一个 管道符号 `|` , 后续的管道命令放在第二个 EOF之后
cat <<EOF |
{
  "name1"   : "$HOME",
  "name2"   : 222
