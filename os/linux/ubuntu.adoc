* http://www.hplipopensource.com/hplip-web/install/install/index.html[打印]
* https://packages.ubuntu.com/[packages.ubuntu.com/]
...
ubuntu 所有的版本号以及 https://wiki.ubuntu.com/DevelopmentCodeNames[codeName] :
...
参考http://wiki.ubuntu.com.cn/%E5%AD%97%E4%BD%93[这里]
...
apt-cache show golang-go   # 查看详细信息
# 会显示很多内容
apt search git
# 列出名称是 git 开头的
apt list 'git*'
...
|===
|hot key |descrption
|Alt{plus}1 |主键盘上的额数字键，可以快速切换至第N个标签页
|Ctrl{plus}Shift{plus}T |开启新的标签页
|Ctrl{plus}Shift{plus}C |复制
|Ctrl{plus}Shift{plus}V |粘贴
|Ctrl{plus}E |清屏
|Ctrl{plus}R |搜索历史命令
|Ctrl{plus}W |向后删除一个词
|Ctrl{plus}C |取消当前行的输入，新开始一行
|Ctrl{plus}U |清空当前输入行
|Ctrl{plus}D |如果当前行是空白行时，可以退出登录，直到退出当前窗口。
|===
...
NOTICE: 该浏览器中文乱码可以参考https://code.google.com/p/chromium/issues/detail?id=316723&q=chinese%20title&colspec=ID%20Pri%20M%20Iteration%20ReleaseBlock%20Cr%20Status%20Owner%20Summary%20OS%20Modified[这里]
...
=== 安装Oracle http://askubuntu.com/questions/56104/how-can-i-install-sun-oracles-proprietary-java-6-7-jre-or-jdk[JDK]
...
参考http://blog.csdn.net/beyond_ray/article/details/38966251[这里]
...
* Lununtu 截屏 https://help.ubuntu.com/community/Lubuntu/Keyboard[Lununtu Keyboard]
...
火狐安装的版本一般都比较低，而Chromium浏览器自带一般相对高些。具体可以通过该https://www.adobe.com/software/flash/about/[网页]检测查看。
...
参考http://www.sudo-juice.com/how-to-set-a-static-ip-in-ubuntu-the-proper-way/[这里]
...
http://www.winehq.org/[wine] 可以让部分Windows程序运行在Linux环境下，主要原理是其重新实现了Windows的API。

http://wiki.ubuntu.com.cn/Wine[安装参考]
...
see http://askubuntu.com/questions/247501/i-get-failed-to-load-session-ubuntu-2d-when-using-xrdp[this]
