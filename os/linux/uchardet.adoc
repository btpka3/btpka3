https://github.com/BYVoid/uchardet[uchardet]
...
事后弥补总不如事前约定，建议使用 https://editorconfig.org/[editorconfig] 进行配置。 这个被众多IDE 工具支持的。
...
PS: 该文件只主要配置 缩进使用空格还是 “, 以及换行符，并不牵涉文件编码。 文件编码还是统一使用 maven，gradle 的工具设定。
