...
[arabic]
. 阿里云传统网络中，内网主机无法直接访问外网。
. 阿里云的NAT网关的费用不低，相当于单独一台 阿里云云主机 一年的费用。
...
[arabic]
. 使用 VirtualBox 最小化安装两个 CentOS 7 的虚拟机，分别命名为 "`nat`", "`inner`"
+
[source,txt]
----
www ━━━━━ nat ━━━━━ inner
----
. 虚拟机 "`nat`" 启用前两个网卡，第一个NAT连接模式，第二个 internal 模式。
. 虚拟机 "`inner`" 启用只启用第二个网卡，为 internal 模式。
+
[cols=",,,",options="header",]
|===
|VM | |nat |inner
...
PS: `NIC` 的含义为网卡。
+
FIXME: 只要 nat 主机开启 ipv4 转发，NIC-1 在 external zone， NIC-2 在 internal zone， inner 就可以访问外网了？
. 分别为两个虚拟机的第二个网卡配置静态类型的IP地址。 文件路径为 `/etc/sysconfig/network-scripts/ifcfg-enp0s8`。 参考内容如下：
+
[source,conf]
----
TYPE=Ethernet
...
结论是 失败。 在阿里云主机 ECS1 上 traceroute ECS2，可以看到经过了3个路由器。 故猜测各个同个公司，同个地域中 多个 ECS 的网络拓扑如下：
...
middle (as router)
  ...
  ┗━━━━━━ <net-2> ━━━━━━ inner
----
. nat 与 middle 在同一个网络， middle 与 inner 在另一个网络，但 inner 直接无法ping 到 nat。
...
VLAN 模式貌似只能在同一个路由器下？跨路由器如何处理暂时不会。 VLAN 的网络设计为 172.20.0.0/16, 虚拟机 nat 的ip地址为 172.20.0.1， 虚拟机 inner 的ip地址为 172.20.0.2。

[arabic]
. 在虚拟机 nat 上启用 vlan （虚拟网卡）
+
[source,bash]
----
...
* 需要 middle 主机启用 net.ipv4.ip++_++forward。
* nat 主机需要执行
+
[source,bash]
----
...
* http://blog.redbranch.net/2015/07/30/centos-7-as-nat-gateway-for-private-network/[CentOS 7 as NAT Gateway for Private Network]
* https://devops.profitbricks.com/tutorials/deploy-outbound-nat-gateway-on-centos-7/[eploy Outbound NAT Gateway on CentOS 7]

curl https://CLIENT++_++ID++_++qh-agency-wap:CLIENT++_++PWD++_++qh-agency-wap++_++123456@login.kingsilk.net/local/11300/rs/oauth/token +
-d grant++_++type=client++_++credentials +
-d scope=SERVER +
