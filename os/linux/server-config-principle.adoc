...
[arabic]
.. 使用 `yum` 安装。
.. 使用预编译的 rpm，bin 安装。但一定要确保来源可靠。
.. 使用源码包进行本地编译安装。只有当上述方式不满足、版本较老、或预编译的选项不符合要求时才使用该方式。前两种方式一般都提供好了init.d脚本等，比较方便。
. 全局性的环境变量应配置到 `/etc/profile.d/his.sh`。用户别的环境变量应当配置到 `~/.bashrc` 中
. 安装的服务、应用，需使用各自独立的系统账户运行，不可使用root账户运行。
. 安装的服务、应统一使用 `service xxxServiceName  start++|++stop++|++restart++|++status` 进行起停控制。 该命令需要配置init脚本： `/etc/init.d/xxxServiceName`。 环境变量，运行用的系统用户应尽量在 init.d 脚本中配置。 如果源码编译安装时不提供init.d脚本，需要手动编写，。之后一般需要执行以下命令：
+
[source,bash]
----
...
[arabic]
. rpm 安装类的软件按照其默认的安装路径。比如：nginx，postgresql等。但是主要数据存放应通过环境变量、配置文件单独指定目录。比如 PostgreSql 指定PG++_++DATA环境变量指定到 /data/store/$++{++software-name}/ 。
. 解压类、编译类软件包的安装路径应指定到 /data/software/$++{++software-name}/ 。比如tomcat、redis等
. TC研发中心开发的应用都放到 `/data/app/$++{++software-name}/` 目录下
