http://dev.mysql.com/doc/mysql-repo-excerpt/5.6/en/linux-installation-yum-repo.html[参考]
...
重置密码参考http://dev.mysql.com/doc/refman/5.5/en/resetting-permissions.html[这里]、更新密码格式参考http://code.openark.org/blog/mysql/upgrading-passwords-from-old_passwords-to-new-passwords[这里]
...
MySql中utf8字符集允许的最大长度是3个字节。 但 emoji 表情符号等的UTF8编码则是4个字节，会出错：

....
java.sql.SQLException: Incorrect string value: '\xF0\x9F\x98\x98' for column 'content' at row 1
....

http://dev.mysql.com/doc/refman/5.5/en/charset-unicode-utf8mb4.html[utf8mb4] 是在 mysql 5.5.3 之后才支持的，每个字符允许的最大字节是4个字节。
...
....
. `++[++ERROR++]++ Missing system table mysql.proxies++_++priv; please run mysql++_++upgrade to create it`, `++[++ERROR++]++ Native table 'performance++_++schema'.'cond++_++instances' has the wrong structure`

以root权限执行 `mysql++_++upgrade -u root -p` 即可。
...
阿里云关系数据库的http://help.aliyun.com/view/11108238_13440406.html[性能]
