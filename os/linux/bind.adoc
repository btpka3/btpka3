...
* https://www.isc.org/downloads/[bind9]
* http://wiki.ubuntu.org.cn/Bind9%E5%AE%89%E8%A3%85%E8%AE%BE%E7%BD%AE%E6%8C%87%E5%8D%97[Bind9安装设置指南]
* https://www.digitalocean.com/community/tutorials/how-to-configure-bind-as-a-private-network-dns-server-on-centos-7#prerequisites[How To Configure BIND as a Private Network DNS Server on CentOS 7]
* https://en.wikipedia.org/wiki/Domain_Name_System_Security_Extensions[Domain Name System Security Extensions]
* https://www.dnssec-tools.org/wiki/index.php?title=Installing_DNSSEC-Tools[DNSSEC-Tools]
* https://tools.ietf.org/html/rfc7858[RFC 7858 - Specification for DNS over Transport Layer Security (TLS)]
* https://github.com/bluejekyll/trust-dns[trust-dns]
* https://www.isc.org/downloads/platform/[bind10], 已经停止维护
* http://fedoraproject.org/wiki/Features/BIND10[Features/BIND10]
...
----
. 修改本地域名配置： `/etc/named/named.conf.local`
+
[source,conf]
----
# 正向域名的解析
...
----
. 创建反向解析文件： `vi /etc/named/zones/db.10.128`
+
[source,conf]
----
$TTL    604800
...
.. 追加相应的 "`PTR`" 记录
.. 追加相应的 Serial 值
. 在 `named.conf` 的 `options` 中 "`trusted`" ACL 追加相应的 IP 地址
. 重启服务
+
[source,bash]
----
