...
[arabic]
. 防止vi时使用小键盘造成乱码：Terminal-++>++Features: 选中 Disable application keypad mode
...
nameserver 8.8.4.4
----
. 检查是否启用网络和主机名 `cat /etc/sysconfig/network` :
+
[source,bash]
----
NETWORKING=yes
...
. DNS
+
参考 ： https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/3/html/Reference_Guide/s1-networkscripts-interfaces.html[8.2. Interface Configuration Files] https://ma.ttias.be/centos-7-networkmanager-keeps-overwriting-etcresolv-conf/[2]
+
[source,bash]
----
...
----
.. centos 7 静态IP `vi /etc/sysconfig/network-scripts/ifcfg-eth0` 。 多IP地址配置请参考 http://netidy.com/blog/add-multiple-ips-network-adapter-centos-7[这里]
+
....
...
DNS1=192.168.7.1
....
.. centos 6 动态IP `cat /etc/sysconfig/network-scripts/ifcfg-eth0`
+
[source,bash]
----
DEVICE="eth0"
...
. 使用代理（若无请跳过）
+
http://www.cyberciti.biz/faq/linux-unix-set-proxy-environment-variable/[参考]
+
[source,bash]
----
[root@h01 ~]# vi /etc/profile.d/custom.sh
