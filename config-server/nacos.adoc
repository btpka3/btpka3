...
- com.alipay.sofa.jraft.rpc.impl.GrpcClient.#link:https://github.com/sofastack/sofa-jraft/blob/322935562b910d415e25493c6ebe6f76713f55b2/jraft-extension/rpc-grpc-impl/src/main/java/com/alipay/sofa/jraft/rpc/impl/GrpcClient.java#L212[newChannel]
...
### 7848
### 8848
alibaba nacos 默认端口 link:https://github.com/alibaba/nacos/blob/8cd5a5ea0bfb7014523eb19ca72e28282a9155a2/naming/src/main/resources/application.properties#L16[HTTP 端口]。

### 9848
针对 GRPC SDK 服务端口
com.alibaba.nacos.api.common.Constants#SDK_GRPC_PORT_DEFAULT_OFFSET = 1000
com.alibaba.nacos.core.remote.grpc.GrpcSdkServer
link:https://github.com/alibaba/nacos/blob/8cd5a5ea0bfb7014523eb19ca72e28282a9155a2/core/src/main/java/com/alibaba/nacos/core/remote/BaseRpcServer.java#L60[com.alibaba.nacos.core.remote.BaseRpcServer]

link:(https://github.com/alibaba/nacos/blob/8cd5a5ea0bfb7014523eb19ca72e28282a9155a2/core/src/main/java/com/alibaba/nacos/core/remote/grpc/BaseGrpcServer.java#L91)[com.alibaba.nacos.core.remote.grpc.BaseGrpcServer#startServer]
...
[source,shell]
----
# PS 下面命令使用的内部docker镜像仓库地址，docker hub的应该是 docker.io/nacos/nacos-server:v3.1.1

podman run --rm -it \
 --platform=linux/amd64 \
 -p 8848:8848 -p 9848:9848 -p 9849:9849 -p 7848:7848 -p 8080:8080 \
 -e MODE=standalone \
 -e NACOS_AUTH_IDENTITY_KEY=nacos \
 -e NACOS_AUTH_IDENTITY_VALUE=nacos \
 -e NACOS_AUTH_TOKEN=SecretKey012345678901234567890123456789012345678901234567890123456789 \
 o-docker.alibaba-inc.com/docker.io/nacos-server:v3.1.1
...
== 本地缓存/cache
${HOME}/snapshots/nacos/config

== 日志/log

${HOME}/logs/nacos/config.log
${HOME}/logs/nacos/naming.log
${HOME}/logs/nacos/remote.log


[source,plain]
----
# 关键词 `[notify-context]` : 有更新，不涉及listener
2025-12-17 21:27:02.861 INFO [nacos.client.config.listener.task-0:c.a.n.c.c.i.CacheData] [fixed-mse-329fed52-nacos-ans.mse.aliyuncs.com_8848] [notify-context] dataId=com.alibaba.security.tenant.common.service.RequestService, group=mapping,tenant=, md5=e18312c9af3aaaadce61974208efb5f1

# 关键词 `[notify-ok]` : 通知更新, 含单个 listener 信息
2025-12-17 21:27:02.861 INFO [nacos.client.config.listener.task-0:c.a.n.c.c.i.CacheData] [fixed-mse-329fed52-nacos-ans.mse.aliyuncs.com_8848] [notify-ok] dataId=com.alibaba.security.tenant.common.service.RequestService, group=mapping,tenant=, md5=e18312c9af3aaaadce61974208efb5f1, listener=org.apache.dubbo.metadata.store.nacos.NacosMetadataReport$NacosConfigListener@35184f3a ,job run cost=0 millis.

# 关键词 `[publish-single]` : 发布服务
2025-12-17 21:26:27.033 INFO [DubboSaveMetadataReport-thread-1:c.a.n.c.c.i.ClientWorker] [fixed-mse-329fed52-nacos-ans.mse.aliyuncs.com_8848] [publish-single] ok, dataId=com.your.company.XxxService:2.0.0:HSF:provider:jiugong-container, group=dubbo, tenant=
----
...
[source,shell]
----
# cacheMap.value() 的key 的格式是 "${dataId}+${group}+${tenant}"

sc -d com.alibaba.security.gong9.mw.pandora.diamond.impl.nacos.NacosFactoryInstance
ognl -c 429b83ba '
#cs =@com.alibaba.security.gong9.mw.pandora.diamond.impl.nacos.NacosFactoryInstance@getGlobalConfigService(),
#cs.namespace,
#cs.worker.cacheMap.value.keySet(),
#cacheData = #cs.worker.cacheMap.value.get("com.alibaba.vbs.generic.hsf.dataid+vbs+8b833a04-ea54-4ef3-ab54-04c885ca4e20"),
#cacheData.listeners
{
#cacheData.envName,
#cacheData.dataId,
...
LDS（Listener Discovery Service）、RDS（Route Discovery Service）和CDS（Cluster Discovery Service）是Envoy代理用于动态配置的重要xDS协议之一


=== LDS (Listener Discovery Service)
LDS用于发现监听器（Listener）。监听器是一个网络端点（例如，一个IP地址和端口），它能够接受连接请求。通过LDS，Envoy可以动态地添加、修改或删除监听器配置。

比如 启动了一个 dubbo 服务 的 provider、consumer。

=== RDS (Route Discovery Service)
RDS负责路由发现。它提供了一种机制来动态更新HTTP连接管理器中的路由配置。这使得Envoy可以根据收到的路由配置来决定如何处理HTTP请求。

比如：dubbo 通过服务名，版本号，分组，或者动态逻辑通过流量标签将请求 发送到目标服务上。


=== CDS (Cluster Discovery Service)
CDS用于集群（Cluster）发现。在Envoy中，集群是指一组逻辑上相同的上游服务实例，Envoy将流量路由到这些实例。
通过CDS，Envoy可以获取集群成员的信息，以及如何与这些成员进行通信的配置。
Istio使用这些服务来为部署在其上的微服务提供智能路由、负载均衡、服务间认证等功能。
通过这些xDS API，Istio可以动态地管理和调整服务网格内服务之间的通信方式，提高系统的灵活性和可维护性。
