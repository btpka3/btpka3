...
----
. 创建/修改 $ZK++_++HOME/conf/zoo.cfg 配置文件。注意：该配置文件不可使用行尾注释
+
[source,ini]
----
clientPort=2110
...
----
. `vi $++{++ZK++_++HOME}/conf/log4j.properties`
+
[source,ini]
----
...
[arabic]
. `vi /usr/local/zookeeper/zookeeper-3.4.6/bin/zkServer.sh`
+
....
# 在最开始追加以下语句
. /etc/profile.d/xxx.sh
