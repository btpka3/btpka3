...
所有相关资料均请参考 https://letsencrypt.org/[Let’s Encrypt] 官网。

下面流程是假设 环境是CentOS 7 和Nginx/Tengine, 并使用 https://certbot.eff.org/#centosrhel7-nginx[Certbot]

[arabic]
. 等到客户访问流量最少的时间段，比如凌晨，如有可能的话，请提前通知客户。
. 登录域名服务商的管理后台
[arabic]
.. 记录并备份要新建/renew(更新)https证书的域名的原有配置。
.. 修改要 新建/renew(更新)https证书的域名（不管是A记录，还是CNAME记录，使之最终查询到的IP为你管理服务器的公网IP）
. 登录到服务器，
[arabic]
.. 初次安装（仅一次）
+
...
* https://ietf-wg-acme.github.io/acme/draft-ietf-acme-acme.html[acme protocol]
...
see https://certbot.eff.org/docs/install.html#running-with-docker[Running with Docker]
