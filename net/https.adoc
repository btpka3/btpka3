...
* http://freessl.wosign.com/freessl[[line-through]#WoSign#] ——因操作不规范已经被要求整改， 火狐等浏览器也将 http://mozilla.com.cn/forum.php?mod=viewthread&tid=373719[移除] 其相关证书。
* https://letsencrypt.org/[Let’s Encrypt] 就是时间短了点,需要3个月renew一下.
* https://www.startssl.com/[startssl.com] 问: 各大https证书厂商推的 普通, DV, OV, EV 证书差别在何处?

== http2

https://http2.akamai.com/demo[demo]
...
* https://www.openssl.org/docs/man1.1.0/apps/[OpenSSL]
* https://www.openssl.org/docs/man1.1.0/apps/x509v3_config.html[x509v3++_++config]
* http://www.rabbitmq.com/ssl.html[RabbitMQ SSL]
* http://slproweb.com/products/Win32OpenSSL.html[Win32 OpenSSL]
* 《http://www.akadia.com/services/ssh_test_certificate.html[How to create a self-signed SSL Certificate]》
* http://conshell.net/wiki/index.php/OpenSSL_to_Keytool_Conversion_tips%20REM%20X.509[OpenSSL to Keytool Conversion tips]
* http://en.wikipedia.org/wiki/X.509#Certificate++_++filename++_++extensions
* https://en.wikipedia.org/wiki/Lightweight_Directory_Access_Protocol[RDN&#44; Relative Distinguished Names]
...
[arabic, start=0]
. 生成一个含自签名 CA 证书的 p12 类型的 KeyStore ++```++bash keytool -genkeypair +
-alias tomcat +
-keyalg RSA +
-keysize 2048 +
-sigalg SHA1withRSA +
...
示例环境：Windows {plus} JDK 1.6 {plus} Tomcat 6
...
....
+
注意：经测试发现WildCard证书无法对 `++*++.localhost` 起作用。而 `++*++.localhost.me` 也无法对 `a.b.localhost.me` 起作用，参见http://security.stackexchange.com/a/26050[这里]。
. 生成自签名的数字证书
[arabic]
.. 针对子域名
+
...
....
+
注意：`-ext` 选项是 http://docs.oracle.com/javase/7/docs/technotes/tools/solaris/keytool.html[JDK 7] 中新增选项 其中CN也应该为IP地址。
. 导出证书
...
. 配置Tomcat启用HTTPS。$++{++CATALINA++_++HOME}/conf/server.xml：
...
....
.. 如果需要单独启动tomcat，则在 %CATALINA++_++HOME%/bin/catalina.bat 开头追加：
+
....
...
参考http://stackoverflow.com/questions/2224066/how-to-convert-ssh-keypairs-generated-using-puttygenwindows-into-key-pairs-use[这里]，使用以下步骤：
...
. Copy your private key to ~/.ssh/id++_++dsa (or id++_++rsa).
. Create the RFC 4716 version of the public key using ssh-keygen `ssh-keygen -i -f ~/.ssh/id++_++dsa++_++com.pub ++>++ ~/.ssh/id++_++dsa.pub`
...
X.509 ： 用于定义公钥的格式 DER ： 用于存储 X.509 证书、PKCS8 私钥等，但其是二级制格式，无法文本编辑器打开进行Review。 PKCS8 ： 用于定义私钥的格式 PEM ： 是 将 DER 内容 base-64 编码

=== PKCS : Public-Key Cryptography Standards

* https://baike.baidu.com/item/PKCS/1042350?fr=aladdin[PKCS]
* https://datatracker.ietf.org/doc/html/rfc8017[PKCS #1: RSA Cryptography Specifications Version 2.2]
* https://datatracker.ietf.org/doc/html/rfc5208[Public-Key Cryptography Standards (PKCS) #8:Private-Key Information Syntax Specification Version 1.2] PKCS#8 用以保存单个秘钥（不含公钥）
* https://datatracker.ietf.org/doc/html/rfc7292[PKCS #12: Personal Information Exchange Syntax v1.1]

=== SPKI : Simple public key infrastructure

* https://theworld.com/~cme/html/spki.html[SPKI/SDSI Certificates]

== cipher suit

=== server https cipher suite

see nmap.md # SSL server cipher suite

=== curl https cipher suites

see curl.md # client ssl cipher suites

=== java client 1

* https://github.com/adoptium/temurin-build/issues/3002[Missing ECDHE Ciphers in 8-jdk-alpine #3002]
