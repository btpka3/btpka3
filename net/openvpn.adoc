...
[arabic]
. VirtualBox 最小化安装 CentOS7 虚拟机 nat, middle, inner 共三个。
[arabic]
.. 虚拟机 nat 启用两个网卡，第一个可以访问外网
.. 虚拟机 middle 启用两个网卡，角色为路由器，连接位于两个网络下的虚拟机 nat，inner。 middle 角色为阿里云上的主机，我们自己无法对它进行任何配置。
.. 虚拟机 inner 启用一个网卡，只跟 middle 相连，不能直接访问公网，与 nat 主机不在一个网络
. 计划网络拓扑如下：
+
[source,txt]
----
...
OpenVPN 的默认配合是会被墙的。因为会被嗅探到 握手信息。需要配置 上 obfsproxy, over SSH 或者 over stunnel。 see https://vpnreviewer.com/vpn-protocols-that-work-in-china[this]
...
----
. `/etc/openvpn/server.conf` 修改内容如下
+
[source,bash]
----
port 1194
...
* openvpn client 端路由表中 `0.0.0.0/1` 即 `0.0.0.0/0.0.0.1` 是什么含义？ 按照 RFC 3330 (reserved networks) 和 RFC 1700 (assigned numbers)， 0.0.0/8 (0.0.0.0 - 0.255.255.255) 的含义是 "`this network`"。 该网络下的任何一个IP地址都不是一个合法的主机IP地址，但却是合法的 来源地址。

== 参考

* https://openvpn.net/[openVpn]
* 《https://www.digitalocean.com/community/tutorials/how-to-setup-and-configure-an-openvpn-server-on-centos-7[How To Setup and Configure an OpenVPN Server on CentOS 7]》
* https://github.com/hwdsl2/docker-ipsec-vpn-server/blob/master/README-zh.md[Docker 上的 IPsec VPN 服务器]
