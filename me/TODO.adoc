...
* mobile app with [line-through]#ionic and angular# cordova and angular material
* 图片仓库（cdn）
* 另外一个 iconfont
* 提供 手机号／电子邮箱　注册账户的一览
...
* Spring Security {plus} CAS for Ajax login status test
...
* Comted sample
* Spring Integration
* Spring http://docs.spring.io/spring-ldap/docs/current/reference/[LDAP]
** Jenkins (LDAP {plus} Auth)
** Maven Nexus (LDAP {plus} Auth)
** SonarQube (LDAP {plus} Auth)
** VisualSVN (LDAP {plus} Auth)
* Scriptable browser plugin
* Linux login {plus} LDAP
...
* http://clouda.baidu.com/portal[Clouda{plus}]
* 自建融合支付
* 自建融合短信网关（云片网，阿里大于）
* OAuth0 中国版
* socks5 代理服务器{plus}浏览器插件
* 微信、QQ报名
* 短信验证码安全监测
* 手机号注册过哪些网站？
* let’s encrypt -dns challenge {plus} aliyun dns api
* nginx traffic monitor SPA
* study
** https://flink.apache.org/[Apache Flink]
** http://www.jstorm.io/index_cn.html[JStorm]
** http://storm.apache.org/[Apache Storm]
** http://spark.apache.org/[Apache Spark]
** http://rocketmq.apache.org/[Apache RocketMQ]
** http://kafka.apache.org/[Apache Kafka]
** hbase
** 实时计算
** 流式处理
** https://dalmatiner.io/[DalmatinerDB]
** http://druid.io/[Druid]
** https://pivotal.io/pivotal-greenplum[greenplum]
...
*** RxJava
*** Reactor vs vert.x
*** https://content.pivotal.io/blog/reacting-to-spring-framework-5-0[Reacting to Spring Framework 5.0]
** https://next.cloudopt.net/#/[Cloudopt]
** http://www.jooq.org/[jooq]
** https://www.jbpm.org/[jBPM]
** https://decisions.com/company
** 时序数据库、监控相关
*** https://micrometer.io/docs[Micrometer]
*** [line-through]#https://docs.spring.io/spring-metrics/docs/current/public/prometheus[Spring Metrics]#
...
*** http://graphite.readthedocs.org/en/latest/[Graphite]
*** http://opentsdb.net/[OpenTSDB]
*** http://kx.com/[Kdb{plus}]
*** http://druid.io/[Druid]
*** http://kairosdb.github.io/[KairosDB]
*** https://github.com/t0nyren/spatedb[Spatedb]
*** link:[Gorilla]
*** https://help.aliyun.com/product/54825.html[HiTSDB]
** cache
*** http://infinispan.org/[Infinispan]
* spring-cloud-zookeeper 中针对 config 对 zookeeper 进行前后端配置。 并按照 spring-boot 加载的顺序进行合并展示。

== 构想：Maven 依赖检查平台

2019年3月份的 https://yida.alibaba-inc.com/s/qmsz[全民扫雷] 中，大家爆了关于 maven 依赖的雷：
...
* "`https://www.atatech.org/articles/134745[BP-基于BOM的Maven项目依赖管理及包冲突解决]`" by 敦行

其他参考： - Maven 类库管理平台： http://mvnmp.alibaba.net/search_log_history.htm[依赖关系查询] - Maven 类库管理平台： http://mvnmp.alibaba.net/edit_mvn_rule.htm[新增依赖规则] - https://www.atatech.org/articles/121763[Maven 插件 maven-enforcer-plugin 使用小结]
...
注册、认证二方包、三方包，维护相关信息： - Owner 的认领与转交 - Aone 信息 - Git 仓库信息 - 二方包使用文档信息 - 外部文档链接 - 传递依赖 的相关信息 维护 - 为什么要用某个该依赖。 - 什么时候要用某个传递依赖（尤其是 scope 是 provide 的传递依赖）。
...
二方包、三方包 Owner 自己按照版本进行打标，全局可见。 非 Owner 打标，仅在相应的应用、应用组中可见。

在二方包/三方包的界面展示标签的时候， 要能区分显示： - 区分显示 平台预设标签 和 自定义标签（按照颜色） - 区分显示 Owner/平台 打的标签 和 应用/应用组内 打的标签
...
分析 POM 检查格式，比如： - 父 POM 的 `++<++relativePath++>++` - 父 POM 中是否按照 子模块的先后依赖顺序排序（aone 中发布二方包时，有先后顺序要求） - 子 POM 中手动写了 插件的版本、依赖的jar的版本 等（应该都在 父POM，BOM 中 管理起来的） - POM 中是否按照 项目/项目组配置 设置相关 properties，jdk 版本、指定插件 等 - POM 中作者、git 仓库，wiki url等信息检查

==== 依赖检查

这部分，有相当一部分可以参考 https://maven.apache.org/enforcer/maven-enforcer-plugin/index.html[maven-enforcer-plugin] 和其扩展 https://www.mojohaus.org/extra-enforcer-rules/[org.codehaus.mojo:extra-enforcer-rules]
...
** enforceBytecodeVersion : 检查二方包自身以及传递依赖的二方包的字节码的最高版本。 这个对 二方包要兼容 jdk 版本（比如 JDK 1.6, 1,7) 时很有用。
...
* 统计二方包被哪些二方包、aone 应用依赖，被使用的版本等。 需要对接一下 aone 等构建工具，目前基本上是只有二方包才发布到 集团内 maven 仓库的，而 应用没有
...
*** https://www.mojohaus.org/extra-enforcer-rules/[org.codehaus.mojo:extra-enforcer-rules]

== FIXME

* 云安全？http://article.liepin.com/ask/qa130273[1]
