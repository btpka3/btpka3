...
# 更更新 ~/.zshrc ，将 /opt/homebrew/Cellar/node/25.9.0_2/bin 目录添加到 PATH 里
if [ -L /opt/homebrew/bin/node ]; then
  _NODE_BIN_DIR="$(cd "$(dirname "$(readlink -f /opt/homebrew/bin/node)")" && pwd)"
  if [ -d "$_NODE_BIN_DIR" ]; then
    # Add the directory where global npm/tnpm packages install executables
    case ":$PATH:" in
      *:"$_NODE_BIN_DIR":*) ;;
...
=== ⭕️⭕️⭕️👍🏻👍🏻👍🏻 fnm / Fast Node Manager

link:https://github.com/Schniz/fnm[github : fnm]
Rust 编写，速度极快（比 nvm 快 10-40 倍）

兼容 nvm 命令语法，迁移成本低
支持 .node-version, .nvmrc 自动切换
...
# 默认安装到 ~/.local/share/fnm/node-versions/
fnm list-remote --lts
fnm install --lts
...
# .nvmrc 文件的内容很简单，就只有个 node 版本号, 比如 : "24.15.0", "v24.15.0", "lts/iron"（不区分大小写，但约定全小写）
----
