...
* https://mp.weixin.qq.com/wiki/10/e5f772f4521da17fa0d7304f68b97d7e.html#.E4.B8.8B.E8.BD.BD.E5.9C.B0.E5.9D.80[微信web开发者工具]
* http://blog.qqbrowser.cc/[参考1]
* 《https://mp.weixin.qq.com/wiki?t=resource/res_main&id=mp1455784140&token=&lang=zh_CN[微信-公众平台：微信web开发者工具]》
* 《http://bbs.browser.qq.com/thread-205291-1-1.html[微信调试工具问题反馈和建议收集贴]》

smb://192.168.0.12/public/wechat++_++web++_++devtools++_++0.7.0++_++x64.exe
...
* 《http://bbs.mb.qq.com/thread-243399-1-1.html?pid=313743&fid=93[微信webview调试方法]》
+
...
. 断点调试 javascript
* 《http://jingyan.baidu.com/article/49711c6151ca75fa441b7c1c.html[linux下android开发真机设备识别不了怎么办]》
...
PS：华为的手机默认没有显示 `开发人员选项`，需要： 手机上 -++>++ 工具 -++>++ 设置 -++>++ 关于手机 -++>++ 连续点击 `版本号` 5～7次， 之后会提示说 进入开发者模式，再返回后就可以看到 `开发人员选项` 了

==== 手机上安装 tbs 调试包

http://res.imtt.qq.com/tbs_inspect/wx_sq_webview_debug.zip[附件]

[arabic]
. 在手机上面安装微信（如果是第一次新装）
. 在微信中随便找个聊天窗口，收入 `//deletetbs` 删除原有的 TBS 工具 (注意：该操作没有任何提示的哦)
. 在手机上安装附件压缩包中的 tbs++_++20150526++_++021257++_++inspector.apk 或者 从http://res.imtt.qq.com/tbs_inspect/TbsSuiteNew.zip[这里]下载。
. 打开 `TBS 工具集` ：
[arabic]
.. 选择 `安装本地TBS内核`
.. 选择 `com.tencent.mm` —— 即为微信安装 TBS
.. 点击最下面的 `快速杀死App（强行停止）` 按钮，退出微信
.. 点击 `(1)安装TBS` 按钮
.. 打开微信，随便找个服务号，公众号的链接，打开一个网址，停留1分钟，然后返回到 TBS 工具集 的画面
.. 点击 `(3)检查是否安装成功`， 确保安装成功
. 将手机通过USB连接到电脑上
. 在手机微信中打开网页
. 吃用 chrominum 浏览器打开 `http://localhost:9222`, 选择要调试的画面进行调试。

==== 电脑上安装 inspector++_++client 脚本

[arabic]
. 电脑上安装 python （略过，Ubuntu貌似已经自带安装了？ PS：注意，Ubuntu下 python 3 的执行命令是 python3, python 2 的执行命令是 python）
. 解压附件中的 `inspector++_++client20150401.zip`
+
....
