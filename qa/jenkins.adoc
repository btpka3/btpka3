...
https://wiki.jenkins-ci.org/display/JENKINS/Installing+Jenkins+on+Red+Hat+distributions[参考]

[arabic]
. 先link:java-jdk-install[安装JDK]
. 新增 jenkins 的 yum 源
+
[source,bash]
----
...
----
. 按照规约修改jenkins的安装目录、存储目录 `bash  mkdir /data/software/jenkins/  chown -R jenkins:jenkins /data/software/jenkins/  mkdir /data/store/jenkins/  mv /usr/lib/jenkins/jenkins.war /data/software/jenkins/  usermod -m -d /data/store/jenkins jenkins               # 修改jenkins用户的主目录为 /data/store/jenkins`
. 修改配置
+
[source,bash]
----
...
https://issues.jenkins-ci.org/browse/JENKINS-12875[405 not a valid crumb was include in the request]
...
https://gist.github.com/wataru420/1757063[ref]

== 升级

RPM可以不用重装的。查看 /etc/init.d/jenkins 可以找到war包的路径。下载新的war包替换即可。

参考 : https://github.com/btpka3/btpka3.github.com/tree/master/os/linux/prod-app-dir/app[线上环境Servlet应用目录结构总结]
