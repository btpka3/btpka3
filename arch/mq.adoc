...
http://mqtt.org/[MQTT] 是一个面向机器的消息通讯协议。
...
Queue 用来存储消息。 Queue 在使用之前，必须先声明。 名称以 `amq.` 开头的 Queue 是 内部保留使用的。

有以下属性： ++*++ Name ++*++ Durable : RabbitMQ 服务重启后 该 Queue 是否还存在 ++*++ Exclusive : 限定只能有一个连接使用该queue，且失去连接时，自动删除该 queue。 ++*++ Auto-delete : 当最后一个 consumer 取消订阅该 queue 后，是否删除该 queue。 ++*++ Arguments : 对实现厂商有意义，比如 TTL。
