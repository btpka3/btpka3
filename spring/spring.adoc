...
spring framework 6.2 : 《link:https://docs.spring.io/spring-framework/reference/6.2/core/beans/java/composing-configuration-classes.html#beans-java-startup-background[Background Initialization]》


github issuse : link:https://github.com/spring-projects/spring-framework/issues/13410[Parallel bean initialization during startup [SPR-8767]]


spring 的这种异步方式需要  需要要修改配置 `@Bean(bootstrap=BACKGROUND)` ，不支持默认全量开启。
...
扫描流程触发顺序
答案：会先触发 getResources()，然后才触发 loadClass()


. 调用 ClassLoader.getResources("basePackage")
. 获取所有匹配的 .class 文件资源（通过 ASM 读取字节码）
. 解析字节码，检查是否包含 @Component 等注解
. 如果需要加载类，才调用 ClassLoader.loadClass()
=== 完整流程示例
...
| 解析 `ChildClass` 字节码
| -
| ASM 读取，发现父类名 `ParentClass`

| 3
| 查找 `ParentClass.class`
| `getResources()`
| 根据父类名查找资源
...
| 解析 `ParentClass` 字节码
| -
| ASM 读取，发现爷爷类名 `GrandParentClass`

| 5
| 查找 `GrandParentClass.class`
| `getResources()`
| 继续向上查找
