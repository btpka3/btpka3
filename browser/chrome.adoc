...
### 参考
* link:https://www.ubuntuupdates.org/ppa/google_chrome?dist=stable[3rd Party Repository: Google Chrome]
* link:https://packages.debian.org/sid/all/ttf-mscorefonts-installer/download[ttf-mscorefonts-installer]
...
## see
* link:https://github.com/SeleniumHQ/docker-selenium/issues/487#issuecomment-302342772[Support Chrome Sandboxing on Ubuntu]
* link:https://gist.github.com/addyosmani/5336747[addyosmani/headless.md]
* link:https://developers.google.com/web/updates/2017/04/headless-chrome[Getting Started with Headless Chrome]
* link:(https://hub.docker.com/r/justinribeiro/chrome-headless/[justinribeiro/chrome-headless]
...
link:https://superuser.com/a/1402013[How can you modify URL predictions on chrome?]

[source,shell]
----
# 只读: 列出 URL 地址栏少量输入时预测的要访问的网址,
# 删除：在url地址栏输入相关词时，可以点击预测网址最后的❌，或按快捷键删除。 快捷键如下:
#      Windows : Shift+Delete,
#      MacOS   : Shift+FN+Delete
# 上述数据保存在 sqllite 数据中, 数据库地址如下
...
# 可以通过 相关数据库工具 修改其中的内容，比如  DBeaver,
# 要查看 History 这个文件/数据库，表: keyword_search_terms,
chrome://predictors/
...
chrome 没有命令行可以列出有多少个 profile，但 MacOS上可以通过以下命令列出有多少profile。
...
for name, info in profiles.items():
    print(f'{name}: {info.get(\"name\", \"Default\")}')
"
----
...
open -a "Google Chrome" --args --profile-directory="Profile 5" \
  --disable-web-security --user-data-dir="$HOME/Library/Application Support/Google/Chrome" \
...
== plugins
=== 👎 XSwitch
* link:https://chromewebstore.google.com/detail/xswitch-v3/dgocjnalnkkghhpdfgjinkhjigkggjdg?hl=en-US&utm_source=ext_sidebar[XSwitch-v3]
* link:https://github.com/yize/xswitch[github: xswitch]
* link:https://chromewebstore.google.com/detail/xswitch/odibcfohggadoanpbhmpcgiebpjpafnl?hl=en-US&utm_source=ext_sidebar[xswitch]

转发后端请求。

=== CORS Unblock

https://chromewebstore.google.com/detail/cors-unblock/lfhmikememgdcahcdlaciloancbhjino?hl=en-US&utm_source=ext_sidebar


== AppleScript 操作 Chrome 的原理
这跟 Chrome DevTools Protocol（CDP，即 debug 协议）无关。
原理是 Chrome 自己实现了 AppleScript 字典（Apple Event / OSA Suite），主动暴露了一组可被脚本调用的接口。可以查看它暴露了什么： 窗口属性、标签页等。
其中关键的是 execute javascript 命令——这是 Chrome 自己提供的 AppleScript 接口，相当于在标签页的控制台里执行 JS。
...
----
  ...
  │             场景              │ 能否读取 │                                  原因                                   │
  ├───────────────────────────────┼──────────┼─────────────────────────────────────────────────────────────────────────┤
  │ Chrome 页面 DOM 文本          │ ✅       │ Chrome 暴露了 execute javascript，等同于在控制台执行 JS，能访问完整 DOM │
  ├───────────────────────────────┼──────────┼─────────────────────────────────────────────────────────────────────────┤
  │ Chrome 页面 <canvas> 内的文字 │ ❌       │ Canvas 是像素画布，innerText 拿不到。需要截图 + OCR                     │
  ├───────────────────────────────┼──────────┼─────────────────────────────────────────────────────────────────────────┤
  │ 其他浏览器（Safari）          │ ✅       │ Safari 也有自己的 AppleScript 字典，支持 do JavaScript                  │
  ├───────────────────────────────┼──────────┼─────────────────────────────────────────────────────────────────────────┤
  │ 其他浏览器（Firefox）         │ ❌       │ Firefox 没有暴露 AppleScript 字典                                       │
  └───────────────────────────────┴──────────┴─────────────────────────────────────────────────────────────────────────┘
----

对于通用 GUI 程序，macOS 有另一套机制叫 Accessibility API（辅助功能），通过 System Events 可以读取。
需要在 系统设置 → 隐私与安全 → 辅助功能 中授权才能用。
...
----
  ...
  │       机制        │              原理              │                         适用范围                         │                        能读什么                        │
  ├───────────────────┼────────────────────────────────┼──────────────────────────────────────────────────────────┼────────────────────────────────────────────────────────┤
  │ AppleScript 字典  │ 应用主动暴露接口               │ 只有实现了字典的应用（Chrome、Safari、Finder、Pages 等） │ 应用自己决定暴露什么，Chrome 暴露了 execute javascript │
  ├───────────────────┼────────────────────────────────┼──────────────────────────────────────────────────────────┼────────────────────────────────────────────────────────┤
  │ Accessibility API │ 系统级辅助功能，读取 UI 控件树 │ 几乎所有 GUI 程序                                        │ 菜单、按钮文字、输入框内容等原生 UI 控件               │
  └───────────────────┴────────────────────────────────┴──────────────────────────────────────────────────────────┴────────────────────────────────────────────────────────┘

  你举的那些例子

  ┌────────────────────────┬───────────────────────┬───────────────────────────────┬─────────────────────────┐
  │          场景          │   AppleScript 字典    │       Accessibility API       │        能拿到吗         │
  ├────────────────────────┼───────────────────────┼───────────────────────────────┼─────────────────────────┤
  │ Chrome DOM 文本        │ ✅ execute javascript │      ❌ 看不到 DOM 内部       │ ✅ 靠 Chrome 自己的接口 │
  ├────────────────────────┼───────────────────────┼───────────────────────────────┼─────────────────────────┤
  │ HTML Canvas 中的文字   │ ❌ 像素，JS 也拿不到  │              ❌               │ ❌ 需要截图+OCR         │
  ├────────────────────────┼───────────────────────┼───────────────────────────────┼─────────────────────────┤
  │ 图片预览中图片里的文字 │          ❌           │ ❌ 只能看到"一个图片视图控件" │ ❌ 需要 OCR             │
  ├────────────────────────┼───────────────────────┼───────────────────────────────┼─────────────────────────┤
  │ 游戏窗口中的文字       │          ❌           │      ❌ 游戏通常自绘渲染      │ ❌ 需要截图+OCR         │
  └────────────────────────┴───────────────────────┴───────────────────────────────┴─────────────────────────┘

  总结： 当时能读到 "Hello World" 不是什么通用魔法，纯粹是因为 Chrome 自己暴露了 execute javascript 这个 AppleScript 命令，本质上等同于在 F12 控制台里执行了 document.getElementById('app').innerText。
----


== cookie 追踪

- chrome://flags ： 设置 privacy-sandbox = Disabled
- chrome://settings/adPrivacy : 三个选项都关闭。
- chrome://settings/cookies ： 选择"阻止第三方 Cookie"  FIXME： 影响？
- AdBlock 插件
- uBlock Origin  插件： 不只是广告拦截，它能基于规则列表直接阻止第三方追踪请求、Cookie 设置脚本的加载，从源头上解决问题，比单纯禁用 Cookie 更有效
