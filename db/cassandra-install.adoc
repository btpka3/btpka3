...
GUI客户端工具 http://dbeaver.jkiss.org/[DBeaver]

http://www.datastax.com/documentation/cassandra/2.1/cassandra/initialize/initializeSingleDS.html

== CentOS 安装

参考http://www.datastax.com/documentation/cassandra/2.0/cassandra/install/installRHEL_t.html[这里]

=== 前提条件

[arabic]
. 已经安装了 JDK 1.7{plus}
. 已经安装了 Python 2.6{plus}
. 已经安装了 JNA 3.2.7{plus} （Java Native Access）
...
|===
|path |description
|/etc/cassandra/conf |CAASSANDRA++_++CONF
|/usr/share/cassandra |CASSANDRA++_++HOME
|/var/lib/cassandra/data |CASSANDRA++_++DATA
|/var/lib/cassandra/commitlog |CASSANDRA++_++COMMITLOG
|===
...
cql介绍请参考http://cassandra.apache.org/doc/cql3/CQL.html[这里]

=== 修改默认认证方式

参考http://www.datastax.com/documentation/cassandra/2.1/cassandra/security/security_config_native_authenticate_t.html[这里]。
...
----
. 增加 system++_++auth keyspace的复制因子（默认为1）
. 重启 Cassandra 客户端。默认超级用户的为 cassandra/cassandra
. 新建一个超级用户，设置复杂、难猜测的密码。
+
[source,bash]
----
...
参考http://www.datastax.com/documentation/cassandra/2.1/cassandra/security/secure_config_native_authorize_t.html[这里]。
