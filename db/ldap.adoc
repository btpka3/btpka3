...
http://msdn.microsoft.com/en-us/library/aa366101(v=vs.85).aspx[DN]是一序列由逗号连接起来的RDN（relative distinguished name）组成。 RDN是 attribute=value 形式的属性值，通常是UTF-8字符串。

* 常见属性名

[width="100%",cols="15%,27%,14%,25%,19%",options="header",]
|===
...
* 保留字符 如果确实需要在DN中使用保留字符，则需要使用反斜杠进行转义。
...
* 路径语法： 如果路径上的DistinguishedName含有保留字符需要使用反斜杠进行转义、或者使用 ’’的形式使用十六进制值。
...
http://en.wikipedia.org/wiki/LDAP_Data_Interchange_Format[LDIF]（LDAP Data Interchange Format）是一种普通文本形式的数据交换格式。用以展示LDAP信息或表示更新请求。
...
LDAP相对于数据库有何优缺点？应当http://www.zytrax.com/books/ldap/ch2/index.html#database[何时]考虑使用LDAP？以下仅作参考：
...
* 不同的objectClass可能有不同的http://www.zytrax.com/books/ldap/ape/index.html#objectclasses[可选、必选attribute]
* objectClass可以继承
...
http://directory.apache.org/apacheds/basic-ug/2.3-introducing-schema.html[schema]是一个打包单元。定义一下内容：
...
参考http://www.zytrax.com/books/ldap/apd/[这里]

LDAP 最佳实践，参考：http://ldapmaven.com/2011/10/27/ldap-programming-best-practices/[1]，http://www.ldapguru.info/ldap/ldap-programming-practices.html[2]
