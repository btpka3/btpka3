...
* http://www.infoq.com/cn/articles/nosql-performance-test[NoSQL性能测试白皮书]
* http://www.csdn.net/article/2012-11-15/2811920-mongodb-quan-gong-lue[MongoDB核心贡献者：不是MongoDB不行，而是你不懂！]
* http://www.csdn.net/article/2015-01-14/2823551[MongoDB：逐渐变得无关紧要]
* http://www.oschina.net/translate/goodbye-mongodb-hello-postgresql?from=20150322[再见 MongoDB，你好 PostgreSQL]
...
see http://docs.mongodb.org/manual/tutorial/install-mongodb-on-ubuntu/[here]
...
centos 安装参考http://docs.mongodb.org/manual/tutorial/install-mongodb-on-red-hat-centos-or-fedora-linux/[这里]
...
使用yum 安装有点冲突问题，先使用 CentOS 7 的预编译的二进制包进行解压安装，参考http://docs.mongodb.org/manual/tutorial/install-mongodb-on-linux/[这里]
...
....
. 配置环境变量 `vi /etc/profile.d/xxx.sh` :
+
....
...
useradd mongod
....
. 创建 systemd 所需的 service 文件 `vi /usr/lib/systemd/system/mongod.service`
+
....
[Unit]
...
[arabic]
. 配置好每台服务器的hostname，并确保每个节点都可以用主机名ping到任意其他节点。比如：修改每个节点的上的 `/etc/hosts`
+
....
192.168.101.81 s81
192.168.101.82 s82
...
192.168.101.85 s85
....
. 在其中一台服务器上创建管理员用户。该服务器应当没有启用认证。使用命令 `mongo` 登录 mongo shell：
+
[source,js]
----
use admin
...
----
. 将 keyfile 复制到规划的 replica set 中的其他服务器上
+
[source,bash]
----
...
----
. 修改每一个 mongodb 的配置文件，设置 replica 参数
+
[source,bash]
----
vi /etc/mongod.conf
...
----
. 使用 `mongo` 命令连接到最开始创建管理员的 mongod 节点上
+
[source,sql]
----
use admin
...
----
. 在 replica set 的每个节点上运行以下命令：
+
[source,js]
----
rs.initiate()
----
. 检查：在 replica set 的每个节点上运行以下命令：
+
[source,js]
----
rs.conf()
...
[arabic]
. 在上述所有对 `/etc/mongod.conf` 修改的基础上，再修改以下设置
+
[source,yaml]
----
storage:
