...
* MyISAM
** 每张表被存为3个文件。_.frm —— 表格定义文件，_.MYD——数据文件，++*++.MY——索引文件
** 查询快
** 不支持事务
** 不支持外键
** 支持全文索引
...
[arabic]
.. 恢复全量备份
.. 在增量从bin-log中从全量的位置开始，恢复到故障发前的position或日期，参考http://dev.mysql.com/doc/refman/5.6/en/point-in-time-recovery.html[这里]。
. 如果在master-slave模式上进行备份和回滚，对应bin-log的File和Position可以通过 `SHOW slave STATUS` 获取，备份前可以 `STOP SLAVE`（而不必像在master上那样必须先锁表）。
...
参考 http://dev.mysql.com/doc/refman/5.6/en/create-procedure.html[create-procedure]、 http://dev.mysql.com/doc/refman/5.6/en/stored-programs-defining.html[stored-programs-defining]

创建存储过程，需要 开启 http://dev.mysql.com/doc/refman/5.6/en/server-system-variables.html#sysvar_log_bin_trust_function_creators[log++_++bin++_++trust++_++function++_++creators] 、或者拥有 `SUPER` 权限。 开启了二进制日期的情况下，还需要声明称成：`DETERMINISTIC`、`NO SQL`、或者 `READS SQL DATA`。
...
http://dev.mysql.com/doc/refman/5.6/en/replication-howto.html[replication-howto]。replication可以做什么？
...
----
. 在session2中查看MASTER的状态，并记录 `File` 和 `Position` 的值
+
[source,bash]
----
...
[arabic]
. master配置 : `vi my.cnf`，如果尚未配置，则修改后需要重启。
+
[source,ini]
----
[mysqld]
...
show slave status \G
----
. 如果同步出现问题，可以参考http://dev.mysql.com/doc/refman/5.0/en/replication-problems.html[这里]进行排查。如果在 slave 上执行`show slave status ++\++G`，且结果中 Slave++_++SQL++_++Running 为 No 时，可以。
[arabic]
.. master: `show master status`，并记下 File 和 Position的值。
.. slave : `stop slave`
.. slave : `RESET SLAVE` – ??? 请自行斟酌是否使用
...
. 必须把ResultSet#close()掉，否则必须会： `Streaming result set com.mysql.jdbc.RowDataDynamic@4447393f is still active. No statements may be issued when any streaming result sets are open and in use on a given connection. Ensure that you have called .close() on any active streaming result sets before attempting more queries.`
...
安装mysql的 http://dev.mysql.com/doc/mysql-apt-repo-quick-guide/en/[apt 仓库]
